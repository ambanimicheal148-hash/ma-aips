...
async function runApi(req, res, url) {
  ...
  try {
    ...
    if (typeof mod.default !== "function") throw new Error("API handler is invalid");
    await mod.default(req, makeResponse(res));
  } catch (error) {
    console.error("API error:", error);
    if (!res.headersSent) { res.statusCode = 500; res.setHeader("Content-Type", "application/json; charset=utf-8"); res.end(JSON.stringify({ error: "MA-AIPS server error" })); }
  }
  return true;
}
...
async function serveStatic(req, res, url) {
  ...
  for (const filePath of candidates) {
    try {
      ...
      res.setHeader("Cache-Control", requestPath === "/index.html" ? "no-cache" : "public, max-age=300"); res.end(data); return;
    } catch {}
  }
...
const server = http.createServer(async (req, res) => {
  ...
  } catch (error) { console.error("Server error:", error); if (!res.headersSent) res.statusCode = 500; res.end("Server error"); }
});
