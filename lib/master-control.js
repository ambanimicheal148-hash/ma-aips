...
export function persistenceStatus() {
  ...
  return { enabled: Boolean(url && token), backend: url && token ? "redis_rest" : "none" };
}
...
export const COMMITMENT_STATES = Object.freeze(["OPEN", "IN PROGRESS", "VERIFIED", "COMPLETED", "FAILED", "EXPIRED"]);
...
export async function recordTask(task, state, extra = {}) {
  ...
  const stateValue = normalized === "IN_PROGRESS" ? "IN PROGRESS" : normalized;
  if (!COMMITMENT_STATES.includes(stateValue)) throw new Error(`Invalid task state: ${state}`);
  const record = { ...task, ...extra, state: stateValue, updatedAt: new Date().toISOString() };
  try {
...
export async function runControlledTask({ message, owner, timeoutMs = DEFAULT_TIMEOUT_MS, execute, verify } = {}) {
  const task = createTask({ message, owner, deadlineMs: timeoutMs });
  await recordTask(task, "OPEN");
  await recordTask(task, "IN PROGRESS");
  ...
  try {
    ...
    const verified = typeof verify === "function" ? await verify(result) : Boolean(result);
    if (!verified) throw new Error("MASTER_AI_VERIFICATION_FAILED");
    await recordTask(task, "VERIFIED", { verification: "explicit_verifier_passed" });
...
export async function updateCommitment(commitmentId, status, actor = "MASTER_AI", extra = {}) {
  assertAuthority(actor, "SPECIALIST");
  if (!COMMITMENT_STATES.includes(String(status).toUpperCase())) throw new Error(`Invalid commitment state: ${status}`);
  return recordTask({ id: String(commitmentId), kind: "COMMITMENT", owner: actor, createdAt: new Date().toISOString(), message: "commitment state update" }, status, extra);
}
