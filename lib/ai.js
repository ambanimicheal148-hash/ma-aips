...
import { interactionPrompt } from "./interaction-context.js";
...
function promptFor(studentId, language, role, memories, message) {
  ...
  var roleRule = role ? "\nDepartment: " + role + "." : "";
  var commanderRule = String(role || "").toUpperCase() === "S_AI_C" || String(studentId || "").toUpperCase() === "S_AI_C"
    ? "\nCOMMANDER NETWORK ACCESS: You are S_AI_C. You may observe and coordinate the full MA.AI.PS AI network, including all registered departments, the security army, and all child products. Use this access for inspection, routing, verification and escalation; do not claim an AI is active unless it is registered and verified." : "";
  return BASE_PROMPT + interactionPrompt(message) + langRule + roleRule + commanderRule + "\nSession: " + (studentId || "unknown") + memory;
}
...
async function groq(msgs, id, lang, role, mem) {
  ...
  var response = await request("https://api.groq.com/openai/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [{ role: "system", content: promptFor(id, lang, role, mem, msgs[msgs.length - 1]?.content || "") }].concat(msgs),
      temperature: 0.7,
      max_tokens: 1000
    })
  });
